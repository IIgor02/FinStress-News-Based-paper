...
\subsection{News-Based FSI Results}

The news-based FSI provides an alternative measure of financial stress derived from professional financial journalism rather than retail investor search behavior. Table~\ref{tab:news_summary} presents descriptive statistics for the news-based index.

\begin{table}[htbp]
\centering
\caption{News-Based FSI Summary Statistics}\label{tab:news_summary}
\begin{tabular}{lc}
\hline
Statistic & Value \\
\hline
Sample Period & Jun 2002 -- Dec 2025 \\
Total Articles Analyzed & 33,881 \\
Articles with Stress Co-occurrence & 6,047 (17.8\%) \\
Weekly Observations & 929 \\
Mean FSI & 0.125 \\
Std. Dev. & 0.124 \\
Min & 0.000 \\
Max & 1.000 \\
...
The news-based FSI exhibits substantial time variation, with a mean of 0.125 and standard deviation of 0.124 on the normalized $[0, 1]$ scale. Approximately 17.8\% of all articles meet the co-occurrence criteria for stress classification, indicating that financial stress is a recurring but not dominant theme in Brazilian financial journalism.

\subsubsection{Crisis Episode Detection}

Figure~\ref{fig:news_fsi} displays the news-based FSI over our sample period. The index successfully captures major stress episodes, with elevated values during known crisis periods.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{../output/news_fsi_analysis.png}
    \caption{News-Based Financial Stress Index (2002--2025). Panel A shows the weekly FSI with high-stress periods (FSI $>$ 0.5) shaded in red and low-stress periods in green. Panel B displays article counts, distinguishing total articles from stress-classified articles. Panel C shows IBOVESPA realized volatility for comparison.}
    \label{fig:news_fsi}
\end{figure}

Table~\ref{tab:news_peaks} presents the highest-stress weeks in our sample (restricted to weeks with at least 10 articles for robustness). The peak stress weeks align with significant market events, including periods of political uncertainty, fiscal concerns, and market corrections.

\begin{table}[htbp]
\centering
\caption{Highest News-Based FSI Weeks (10+ Articles)}\label{tab:news_peaks}
\begin{tabular}{lccc}
\hline
Week & FSI & Stress Articles & Total Articles \\
\hline
Mar 25, 2019 & 0.634 & 7 & 10 \\
Apr 22, 2024 & 0.615 & 8 & 11 \\
Jun 19, 2023 & 0.512 & 13 & 18 \\
Apr 9, 2007 & 0.510 & 7 & 12 \\
Jun 9, 2014 & 0.473 & 27 & 41 \\
\hline
\end{tabular}
\end{table}

\subsubsection{Comparison with Google Trends FSI}

The news-based and Google Trends-based FSI measures capture complementary aspects of financial stress. The Dictionary FSI reflects retail investor attention and concern, while the News FSI captures the intensity of stress-related coverage in professional financial media.

The Granger causality results in Table~\ref{tab:granger} indicate that the Dictionary FSI Granger-causes the News FSI ($p = 0.004$), suggesting that retail investor search behavior leads professional media coverage. This finding is consistent with the information diffusion hypothesis: retail investors may react to early stress signals before journalists compile and publish their analysis.

\subsubsection{Methodological Implications}

The news-based approach offers several advantages over search-based measures:

\begin{enumerate}
    \item \textit{Professional filtering}: News articles undergo editorial review, potentially reducing noise from irrelevant searches
    \item \textit{Longer sample period}: News archives extend further back than Google Trends data (2002 vs. 2008)
    \item \textit{Content depth}: Full article text provides richer context than search query counts
\end{enumerate}

However, the news-based FSI also has limitations. Coverage intensity varies by news source, and editorial decisions may introduce selection bias. The two-way co-occurrence method, while more inclusive than three-way co-occurrence, may still miss nuanced stress discussions that do not use explicit dictionary terms.
